-- KASAI Supabase schema
-- Run this whole file in the Supabase SQL Editor.
-- It is idempotent and keeps Row Level Security enabled for every user table.
...
create table if not exists public.user_sync_events (
  ...
  device_id text,
  device_label text,
  status text not null check (status in ('success','error','pending','offline')),
...
alter table public.profiles alter column step_goal drop not null;
alter table public.profiles alter column step_goal drop default;
alter table public.profiles alter column kcal_goal drop not null;
alter table public.profiles alter column kcal_goal drop default;
alter table public.profiles alter column water_goal drop not null;
alter table public.profiles alter column water_goal drop default;
alter table public.profiles alter column protein_min drop not null;
alter table public.profiles alter column protein_min drop default;
alter table public.profiles alter column protein_max drop not null;
alter table public.profiles alter column protein_max drop default;
...
begin
  if not exists (
    ...
    where conname = 'user_sync_events_status_check'
      and conrelid = 'public.user_sync_events'::regclass
  ) then
    alter table public.user_sync_events
      add constraint user_sync_events_status_check
      check (status in ('success','error','pending','offline'));
  end if;
end $$;
...
grant select, insert, update, delete on public.nutrition_logs to anon;
...
grant select, insert, update, delete on public.user_app_state to anon;
grant select, insert, update, delete on public.user_device_locks to authenticated;
grant select, insert, update, delete on public.user_device_locks to anon;
grant select, insert, delete on public.user_sync_events to authenticated;
grant select, insert, delete on public.user_sync_events to anon;
...
create trigger nutrition_logs_set_updated_at before update on public.nutrition_logs
for each row execute function public.set_updated_at();
...
drop trigger if exists user_device_locks_set_updated_at on public.user_device_locks;
create trigger user_device_locks_set_updated_at before update on public.user_device_locks
for each row execute function public.set_updated_at();
...
grant execute on function public.acquire_device_lock(text, text, boolean, integer) to authenticated;
grant execute on function public.acquire_device_lock(text, text, boolean, integer) to anon;
grant execute on function public.heartbeat_device_lock(text) to authenticated;
grant execute on function public.heartbeat_device_lock(text) to anon;
grant execute on function public.release_device_lock(text) to authenticated;
grant execute on function public.release_device_lock(text) to anon;
